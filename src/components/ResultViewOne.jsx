import '../styles/result1.css'
import { useRef, useState } from 'react'
import { motion } from 'framer-motion'
import { toBlob } from 'html-to-image'
import ShareSheet from './ShareSheet.jsx'
import templateSrc from '../assets/temp1.png'
import { downloadBlob, postToX, nativeShare, canUseNativeShare } from '../lib/shareUtils.js'

const FILENAME = 'hacker-house-goa-2026-builder-id.png'
const CAPTION = "I'm building at Hacker House Goa 2026 🏝️ #FrameInGoa"

/**
 * ResultViewOne
 * Card layout matches the updated temp1.png artwork: one large photo
 * panel on the left, two fields on the right (Name, Tech stack) — no
 * email field, no pink tags baked into the art, no in-card captions.
 * Download/Share rasterize the live card DOM to a PNG via html-to-image,
 * then hand off to the same downloadBlob / postToX / nativeShare helpers
 * ResultView uses.
 *
 * npm install html-to-image
 */
export default function ResultViewOne({
    photoUrl = null,
    name = '',
    techStack = '',
    builderClass = '',
                ...
                withPhotoAsDataUrl(async () => {
                    ...
                    const blob = await withTimeout(
                        toBlob(node, {
                            ...
                            // `aspect-ratio` on .idcard1 often doesn't resolve
    ...
    return (
        <div className="result-panel">
            ...
            >
                ...
                {/* Photo */}
                <div className="idcard1__photo">
                    {hasPhoto ? (
                        <img ref={photoImgRef} src={photoUrl} alt={name ? `${name}'s photo` : 'Builder photo'} />
                    ) : (
                        <div className="idcard1__photo-placeholder">
                ...
                {/* Field 1: Name */}
                <span className="idcard1__tag idcard1__tag--1">Name</span>
                <div className="idcard1__box idcard1__box--1">
                    <span className={`idcard1__value${name ? '' : ' idcard1__value--placeholder'}`}>
                        {name || 'Your name'}
                    </span>
                </div>

                {/* Field 2: Tech stack */}
                <span className="idcard1__tag idcard1__tag--2">Tech stack</span>
                <div className="idcard1__box idcard1__box--2">
                    <span className={`idcard1__value${techStack ? '' : ' idcard1__value--placeholder'}`}>
                        {techStack || 'e.g. React + Node'}
                    </span>
                </div>

                {/* Footer builder class label */}
                <span className={`idcard1__footer-label${builderClass ? '' : ' idcard1__footer-label--placeholder'}`}>
