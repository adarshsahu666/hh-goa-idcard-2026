import '../styles/result3.css'
import { useRef, useState } from 'react'
import { motion } from 'framer-motion'
import { toBlob } from 'html-to-image'
import ShareSheet from './ShareSheet.jsx'
import templateSrc from '../assets/temp3.png'
import { downloadBlob, postToX, nativeShare, canUseNativeShare } from '../lib/shareUtils.js'

const FILENAME = 'hacker-house-goa-2026-builder-id-v3.png'
const CAPTION = "I'm building at Hacker House Goa 2026 🏝️ #FrameInGoa"

/**
 * ResultViewFour
 * Card layout matches the temp3.png artwork: a circular photo
 * frame centered near the top, then two pill-shaped field bars
 * below it — a pink bar with a person icon for Name, and a gold
 * bar with a "</>" icon for Tech stack. Like idcard3, none of this
 * assumes the template PNG contains the icon chips or input
 * fields; they're drawn in CSS (result3.css) so the card renders
 * correctly regardless of the background art underneath.
 *
 ...
 */
export default function ResultViewFour({
    photoUrl = null,
    name = '',
    techStack = '',
    builderClass = '',
                ...
                withPhotoAsDataUrl(async () => {
                    ...
                    const blob = await withTimeout(
                        toBlob(node, {
                            ...
                            // `aspect-ratio` on .idcard4 often doesn't resolve
    ...
    return (
        <div className="result-panel">
            ...
            >
                ...
                <div className="idcard4__photo">
                    {hasPhoto ? (
                        <img ref={photoImgRef} src={photoUrl} alt={name ? `${name}'s photo` : 'Builder photo'} />
                    ) : (
                        <div className="idcard4__photo-placeholder">
                ...
                <div className="idcard4__field idcard4__field--1">
                    <span className="idcard4__field-icon idcard4__field-icon--pink" aria-hidden="true">
                    ...
                    <span className="idcard4__field-input">
                        <span className={`idcard4__value${name ? '' : ' idcard4__value--placeholder'}`}>
                            {name || 'Your name'}
                        </span>
                    </span>
                </div>

                {/* Field 2: Tech stack — gold chip + code icon */}
                <div className="idcard4__field idcard4__field--2">
                    <span className="idcard4__field-icon idcard4__field-icon--gold" aria-hidden="true">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                            <path
                    ...
                    <span className="idcard4__field-input">
                        <span className={`idcard4__value${techStack ? '' : ' idcard4__value--placeholder'}`}>
